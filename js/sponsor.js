...
async function applyVisibilitySettings() {
    ...
    try {
        ...
        if (result.status === 'success') {
            ...
            const mapping = {
                'sponsor_hero': '#sponsor-hero',
                'sponsor_upcoming': '#upcoming-races',
                'sponsor_tiers': '#sponsor-tiers',
                'sponsor_active': '#active-sponsors-section',
                'sponsor_inquiry': '#form-container',
                'sponsor_support': '#sponsor-support'
            };

            Object.entries(mapping).forEach(([settingId, selector]) => {
                if (settings[settingId] === 'Hide') {
                    const el = document.querySelector(selector);
                    if (el) el.style.display = 'none';
                }
            });
            
            // If both inquiry and support are hidden, hide their parent container
            if (settings['sponsor_inquiry'] === 'Hide' && settings['sponsor_support'] === 'Hide') {
                const joinQuest = document.getElementById('join-quest');
                if (joinQuest) joinQuest.style.display = 'none';
            }
        }
    } catch (error) {
...
async function loadAbbottMajors() {
    const container = document.getElementById('majors-dynamic-container');
    if (!container) return;
    ...
    try {
        ...
        if (result.status === 'success' && result.data) {
            ...
            if (majors.length === 0) {
                container.innerHTML = '<p class="text-xs text-on-surface-variant/60">No goals set yet.</p>';
                return;
            }
            ...
            container.innerHTML = majors.map(major => {
                ...
                return `
                    <div class="flex flex-col items-center shrink-0 sm:shrink min-w-[80px]">
                        <span class="material-symbols-outlined text-4xl sm:text-5xl mb-2 sm:mb-4 ${isCompleted ? 'star-active' : 'star-pending'}">star</span>
                        <span class="text-sm font-heading font-bold text-on-surface">${major.Name}</span>
                        <span class="text-[9px] font-heading font-bold uppercase tracking-[0.18em] ${isCompleted ? 'text-primary' : 'text-outline-variant'} mt-1" 
                              style="${isCompleted ? 'font-weight:800' : ''}">
        ...
        console.error('Error loading dynamic Abbott Majors:', error);
        container.innerHTML = '<p class="text-xs text-error">Sync Error</p>';
    }
}

async function loadCampaignSettingsPrices() {
    try {
        ...
        if (result.status === 'success' && result.data) {
            const settings = result.data;
            const pricingMap = {
                'tier_pacer_price': ['price-pacer', 'matrix-price-pacer'],
                'tier_gear_price': ['price-gear', 'matrix-price-gear'],
                'tier_voyager_price': ['price-voyager', 'matrix-price-voyager'],
                'tier_star_price': ['price-star', 'matrix-price-star']
            };
            for (const [key, elementIds] of Object.entries(pricingMap)) {
                if (settings[key]) {
        ...
        console.error("Error loading tier prices:", e);
    }
}

async function loadActiveSponsors() {
    const section = document.getElementById('active-sponsors-section');
    ...
    try {
        ...
        if (result.status === 'success' && result.data && result.data.length > 0) {
            ...
            // Define tier ordering: Star Partner -> The Voyager -> Gear Master -> The Pacer
            const tierOrder = {
                'Star Partner': 4,
                'The Voyager': 3,
                'Gear Master': 2,
            ...
            // Sort by tier importance, then by brand name
            sponsors.sort((a, b) => {
                const orderA = tierOrder[a.Tier] || 0;
                const orderB = tierOrder[b.Tier] || 0;
                if (orderB !== orderA) return orderB - orderA;
                return (a.BrandName || '').localeCompare(b.BrandName || '');
            });
            
            grid.innerHTML = sponsors.map(s => {
                // Determine size or style based on Tier
                let scaleClass = 'h-16 w-32';
                if (s.Tier === 'Star Partner') {
                    scaleClass = 'h-24 w-48 border border-outline-variant p-2 shadow-md bg-white';
                } else if (s.Tier === 'The Voyager') {
                    scaleClass = 'h-20 w-40 border border-outline-variant/60 p-2 bg-white';
                } else {
                    scaleClass = 'h-16 w-32 border border-outline-variant/40 p-2 bg-white';
                }
                
                const linkStart = s.Link ? `<a href="${s.Link}" target="_blank" class="block transition-transform hover:scale-105" title="${s.BrandName} (${s.Tier})">` : '<div class="transition-transform hover:scale-105">';
                const linkEnd = s.Link ? '</a>' : '</div>';
                
                return `
                    ${linkStart}
                        <div class="flex flex-col items-center gap-2">
                            <div class="${scaleClass} rounded-xl overflow-hidden flex items-center justify-center p-2 group transition-all duration-300">
                                <img src="${s.LogoUrl}" alt="${s.BrandName}" class="max-w-full max-h-full object-contain transition-all duration-300">
                            </div>
                            <span class="text-[10px] font-heading font-extrabold uppercase tracking-widest text-on-surface-variant/70">${s.Tier}</span>
                        </div>
                    ${linkEnd}
                `;
            }).join('');
        ...
        console.error("Error loading active sponsors:", e);
    }
}
