...
function setupEventListeners() {
    ...
    // Tier Pricing Form
    const tierPricingForm = document.getElementById('tier-pricing-form');
    if (tierPricingForm) tierPricingForm.addEventListener('submit', saveTierPricingSettings);

    // Sponsor Form
    const sponsorForm = document.getElementById('sponsor-form');
    if (sponsorForm) sponsorForm.addEventListener('submit', handleSponsorSubmit);

    // Sponsor Logo File Selection
    const sponsorLogoFile = document.getElementById('sponsor-logo-file');
    if (sponsorLogoFile) sponsorLogoFile.addEventListener('change', handleSponsorLogoSelect);
...
function editRace(id) {
    ...
    const editTimeStr = (race.Time && race.Time !== '-' && race.Time !== '--:--:--') ? race.Time : '';
    let formattedEditTime = editTimeStr;
    const cleanEditTime = String(editTimeStr).replace(/\D/g, '');
    if (cleanEditTime.length >= 6) {
        formattedEditTime = `${cleanEditTime.substring(0, 2)}:${cleanEditTime.substring(2, 4)}:${cleanEditTime.substring(4, 6)}`;
    } else if (cleanEditTime.length >= 4) {
        formattedEditTime = `${cleanEditTime.substring(0, 2)}:${cleanEditTime.substring(2, 4)}`;
    }
    document.getElementById('race-time').value = formattedEditTime;
    document.getElementById('race-time').dataset.rawTime = formattedEditTime;
...
function formatTimeInput(e) {
    ...
    // Keep the HH:MM:SS format for the backend
    e.target.dataset.rawTime = formatted;
...
function formatDisplayTime(timeStr) {
    ...
    if (cleanStr.includes('T') && cleanStr.includes('Z')) {
        try {
            ...
            normalized = `${String(d.getHours()).padStart(2, '0')} Hrs ${String(d.getMinutes()).padStart(2, '0')} Minutes ${String(d.getSeconds()).padStart(2, '0')} Sec`;
            return normalized;
        } catch (e) { }
    } else if (/^\d{6}$/.test(cleanStr)) {
        return `${cleanStr.substring(0, 2)} Hrs ${cleanStr.substring(2, 4)} Minutes ${cleanStr.substring(4, 6)} Sec`;
    }

    if (/^\d{2}:\d{2}:\d{2}$/.test(normalized)) {
        const parts = normalized.split(':');
        return `${parts[0]} Hrs ${parts[1]} Minutes ${parts[2] || '00'} Sec`;
    }
...
async function loadSystemSettings() {
    ...
    const sectionGroups = {
        ...
        'sponsor': {
            title: 'Sponsor Page (sponsor.html)',
            sections: {
                'sponsor_hero': 'Partner Opportunity (Hero)',
                'sponsor_upcoming': 'Upcoming Races',
                'sponsor_tiers': 'Partnership Tiers',
                'sponsor_active': 'Active Sponsors & Partners',
                'sponsor_inquiry': 'Start a Conversation',
                'sponsor_support': 'Support the Athlete'
            }
        },
        'fundraisers': {
            title: 'Fundraisers Page (fundraisers.html)',
            sections: {
                'fund_hero': 'Hero Fundraiser Section',
                'fund_top': 'Top Contributors Leaderboard',
                'fund_table': 'Campaign Supporters Table',
                'fund_share': 'Share the Journey'
            }
        }
    };

    let html = '<table class="w-full text-left border-collapse"><thead><tr class="border-b border-outline-variant text-on-surface-variant text-xs uppercase tracking-wider font-bold"><th class="pb-4 pr-4 pl-4 pt-4">Page</th><th class="pb-4 pr-4 pt-4">Section Name</th><th class="pb-4 pr-4 pt-4">Status</th><th class="pb-4 pr-4 pt-4 text-right">Visibility</th></tr></thead><tbody class="divide-y divide-outline-variant">';

    Object.entries(sectionGroups).forEach(([pageId, group]) => {
        let firstRow = true;
        const sectionsCount = Object.keys(group.sections).length;
        
        Object.entries(group.sections).forEach(([id, name]) => {
            html += `<tr class="group hover:bg-primary/5 transition-athletic opacity-50" id="setting-row-${id}">`;
            
            if (firstRow) {
                html += `<td rowspan="${sectionsCount}" class="py-4 pr-4 pl-4 font-bold align-top border-r border-outline-variant/30 text-on-surface-variant w-1/4">${group.title}</td>`;
                firstRow = false;
            }
            
            html += `
                <td class="py-4 pr-4 pl-4 font-semibold text-on-background w-2/5">${name}</td>
                <td class="py-4 pr-4 text-xs text-on-surface-variant w-1/5 setting-status-text">Syncing...</td>
                <td class="py-4 pr-4 pl-4 text-right">
    ...
    try {
        ...
        if (result.status === 'success') {
            ...
            Object.entries(sectionGroups).forEach(([pageId, group]) => {
                Object.entries(group.sections).forEach(([id, name]) => {
                    // Default to Show if setting is missing (new section)
                    const isVisible = settings[id] === 'Hide' ? false : true;
                    
                    const row = document.getElementById(`setting-row-${id}`);
                    if (row) {
                        row.classList.remove('opacity-50');
                        const statusText = row.querySelector('.setting-status-text');
        ...
        document.querySelectorAll('.setting-toggle').forEach(t => {
            t.disabled = false;
            t.parentElement.parentElement.parentElement.classList.remove('opacity-50');
        });
    }
}
...
function shareThankYou(encodedName, amount, currency) {
    ...
    const modalId = 'custom-share-modal';
    ...
    modal.className = 'fixed inset-0 z-[200] flex items-center justify-center bg-black/80 backdrop-blur-sm px-4';
    ...
    modal.innerHTML = `
        <div class="glass-panel max-w-md w-full p-8 rounded-xl border border-outline-variant relative">
            <button onclick="document.getElementById('${modalId}').classList.add('hidden')" class="absolute top-4 right-4 p-2 hover:text-primary transition-athletic">
                <span class="material-symbols-outlined">close</span>
            </button>
            <h3 class="text-2xl font-display font-bold mb-2">Share Thank You</h3>
            <p class="text-sm text-on-surface-variant mb-6">Choose how to share this message with ${name}.</p>
            
            <div class="bg-surface-container p-4 rounded-lg text-on-background mb-6 whitespace-pre-wrap font-mono text-[11px] overflow-y-auto max-h-40 border border-outline-variant">${message}</div>
            
            <div class="grid grid-cols-2 gap-3 mb-2">
                <button onclick="copyToClipboardFallback(\`${safeMsg}\`, '${safeName}', '${amountFormatted}', '${currency}'); document.getElementById('${modalId}').classList.add('hidden')" class="flex items-center justify-center gap-2 bg-surface-container-highest hover:bg-primary/10 hover:text-primary text-on-background font-bold py-3 rounded-xl transition-athletic border border-outline-variant">
                    <span class="material-symbols-outlined text-[18px]">content_copy</span> Copy Text
                </button>
                <a href="https://wa.me/?text=${encodedMessage}" target="_blank" onclick="logThankYou('${safeName}', '${amountFormatted}', '${currency}', \`${safeMsg}\`); document.getElementById('${modalId}').classList.add('hidden')" class="flex items-center justify-center gap-2 bg-[#25D366]/10 hover:bg-[#25D366]/20 text-[#25D366] font-bold py-3 rounded-xl transition-athletic border border-[#25D366]/30">
                ...
                <a href="https://twitter.com/intent/tweet?text=${encodedMessage}" target="_blank" onclick="logThankYou('${safeName}', '${amountFormatted}', '${currency}', \`${safeMsg}\`); document.getElementById('${modalId}').classList.add('hidden')" class="flex items-center justify-center gap-2 bg-black/10 hover:bg-black/20 text-on-background font-bold py-3 rounded-xl transition-athletic border border-outline-variant">
                    <svg class="w-4 h-4 fill-current" viewBox="0 0 24 24"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>
                    X (Twitter)
                </a>
                <a href="https://www.facebook.com/sharer/sharer.php?u=${encodedUrl}" target="_blank" onclick="logThankYou('${safeName}', '${amountFormatted}', '${currency}', \`${safeMsg}\`); document.getElementById('${modalId}').classList.add('hidden')" class="flex items-center justify-center gap-2 bg-[#1877F2]/10 hover:bg-[#1877F2]/20 text-[#1877F2] font-bold py-3 rounded-xl transition-athletic border border-[#1877F2]/30">
                    <svg class="w-5 h-5 fill-current" viewBox="0 0 24 24"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.469h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.469h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>
                    Facebook
                </a>
                <a href="mailto:?subject=Thank%20You%20for%20your%20Support!&body=${encodedMessage}" target="_blank" onclick="logThankYou('${safeName}', '${amountFormatted}', '${currency}', \`${safeMsg}\`); document.getElementById('${modalId}').classList.add('hidden')" class="flex items-center justify-center gap-2 bg-error/10 hover:bg-error/20 text-error font-bold py-3 rounded-xl transition-athletic border border-error/30 col-span-2">
                    <span class="material-symbols-outlined text-[18px]">mail</span> Send via Email
                </a>
            </div>
        </div>
    `;
...
const sponsorItemsPerPage = 10;
let sponsorLogoBase64 = null;

async function loadSponsors() {
    loadTierPricingSettings();
    
    const tbody = document.getElementById('admin-sponsor-table-body');
    if (!tbody) return;

    try {
        const response = await fetch(`${SCRIPT_URL}?action=getSponsors&_=${Date.now()}`);
        const result = await response.json();

        if (result.status === 'success') {
            allSponsors = result.data || [];
            if (sponsorCurrentPage > Math.ceil(allSponsors.length / sponsorItemsPerPage)) {
        ...
        tbody.innerHTML = '<tr><td colspan="4" class="py-10 text-center text-error font-medium">Unable to load sponsors list.</td></tr>';
    }
}

async function loadTierPricingSettings() {
    try {
        const response = await fetch(`${SCRIPT_URL}?action=getCampaignSettings&_=${Date.now()}`);
        const result = await response.json();
        if (result.status === 'success' && result.data) {
            const settings = result.data;
            if (settings.tier_pacer_price) document.getElementById('price-pacer-input').value = settings.tier_pacer_price;
            if (settings.tier_gear_price) document.getElementById('price-gear-input').value = settings.tier_gear_price;
            if (settings.tier_voyager_price) document.getElementById('price-voyager-input').value = settings.tier_voyager_price;
            if (settings.tier_star_price) document.getElementById('price-star-input').value = settings.tier_star_price;
        }
    } catch (error) {
        console.error("Error loading tier pricing settings:", error);
    }
}

async function saveTierPricingSettings(e) {
    e.preventDefault();
    const btn = document.querySelector('#tier-pricing-form button[type="submit"]');
    ...
    const data = {
        tier_pacer_price: document.getElementById('price-pacer-input').value.trim(),
        tier_gear_price: document.getElementById('price-gear-input').value.trim(),
        tier_voyager_price: document.getElementById('price-voyager-input').value.trim(),
        tier_star_price: document.getElementById('price-star-input').value.trim()
    };
    ...
    try {
        ...
        const result = await response.json();
        if (result.status === 'success') {
            status.textContent = 'Global pricing scales applied successfully!';
            status.className = 'text-[10px] text-center text-primary font-bold mt-1';
        ...
        status.textContent = 'Error: ' + error.message;
        status.className = 'text-[10px] text-center text-error font-bold mt-1';
    } finally {
        btn.disabled = false;
...
async function handleSponsorLogoSelect(e) {
    const file = e.target.files[0];
    if (!file) return;

    const statusEl = document.getElementById('sponsor-logo-status');
    const preview = document.getElementById('sponsor-logo-preview');
    const container = document.getElementById('sponsor-logo-preview-container');
    const nameEl = document.getElementById('sponsor-logo-name');
    const sizeEl = document.getElementById('sponsor-logo-size');

    try {
        const result = await compressImageIfNeeded(file, statusEl);
        sponsorLogoBase64 = result.base64; // Store base64 data
        preview.src = 'data:image/jpeg;base64,' + sponsorLogoBase64;
        
        if (nameEl) nameEl.textContent = file.name;
        if (sizeEl) sizeEl.textContent = `${Math.round(result.compressedSize / 1024)} KB (WebP-Optimized)`;
        
        container.classList.remove('hidden');
    } catch (err) {
        console.error("Sponsor logo compression failed:", err);
    }
}
...
function clearSponsorLogo() {
    ...
    document.getElementById('sponsor-logo-preview-container').classList.add('hidden');
}
...
async function handleSponsorSubmit(e) {
    ...
    const action = id ? 'updateSponsor' : 'createSponsor';
    ...
    const data = {
        id: id,
        BrandName: document.getElementById('sponsor-brand').value.trim(),
        Tier: document.getElementById('sponsor-tier').value,
        Link: document.getElementById('sponsor-link').value.trim(),
        logoBase64: sponsorLogoBase64,
        logoFileName: sponsorLogoBase64 ? `sponsor-${Date.now()}.jpg` : null
    };
...
function renderSponsorsTable() {
    const tbody = document.getElementById('admin-sponsor-table-body');
    const info = document.getElementById('sponsor-pagination-info');
    if (!tbody) return;

    tbody.innerHTML = '';

    if (allSponsors.length === 0) {
        tbody.innerHTML = '<tr><td colspan="4" class="py-10 text-center text-on-surface-variant font-medium">No sponsors registered yet.</td></tr>';
        if (info) info.textContent = 'Showing 0 sponsors';
        return;
    }

    const start = (sponsorCurrentPage - 1) * sponsorItemsPerPage;
    const end = Math.min(start + sponsorItemsPerPage, allSponsors.length);
    const paginated = allSponsors.slice(start, end);

    paginated.forEach(item => {
        const tr = document.createElement('tr');
        tr.className = "group hover:bg-primary/5 transition-athletic";
        
        const logoUrl = item.LogoUrl || 'https://placehold.co/100?text=No+Logo';
        const linkDisplay = item.Link ? `<a href="${item.Link}" target="_blank" class="text-primary hover:underline font-semibold truncate max-w-xs block">${item.Link}</a>` : '<span class="text-on-surface-variant/40 italic">No link</span>';

        tr.innerHTML = `
            <td class="py-4 pr-4">
                <div class="flex items-center gap-3">
                    <img src="${logoUrl}" alt="${item.BrandName}" class="h-10 w-16 object-contain rounded-lg border border-outline-variant bg-white p-1">
                    <div>
                        <p class="font-bold text-sm text-on-background">${item.BrandName}</p>
                        <p class="text-[10px] text-on-surface-variant/60 font-semibold uppercase">${item.Tier}</p>
                    </div>
                </div>
            </td>
            <td class="py-4 pr-4">
            ...
            <td class="py-4 pr-4">
                <span class="text-xs text-on-surface-variant">${item.Timestamp ? new Date(item.Timestamp).toLocaleDateString() : 'N/A'}</span>
            </td>
            <td class="py-4 text-right">
                <div class="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-athletic">
                    <button onclick="editSponsor('${item.id}')" class="p-2 hover:bg-primary/20 text-primary rounded-lg transition-athletic">
                        <span class="material-symbols-outlined text-lg">edit</span>
                    </button>
                    <button onclick="deleteSponsor('${item.id}')" class="p-2 hover:bg-error/20 text-error rounded-lg transition-athletic">
                        <span class="material-symbols-outlined text-lg">delete</span>
                    </button>
                </div>
            </td>
    ...
    if (info) info.textContent = `Showing ${start + 1}-${end} of ${allSponsors.length} sponsors`;
}

function changeSponsorPage(dir) {
    const totalPages = Math.ceil(allSponsors.length / sponsorItemsPerPage);
    if (dir === -1 && sponsorCurrentPage > 1) {
        sponsorCurrentPage--;
        renderSponsorsTable();
    } else if (dir === 1 && sponsorCurrentPage < totalPages) {
        sponsorCurrentPage++;
        renderSponsorsTable();
    }
}
...
function editSponsor(id) {
    ...
    document.getElementById('sponsor-form-title').textContent = 'Update Sponsor';
    document.getElementById('sponsor-submit-btn').textContent = 'Apply Update';
    document.getElementById('sponsor-cancel-btn').classList.remove('hidden');

    document.getElementById('sponsor-id').value = sponsor.id;
    document.getElementById('sponsor-brand').value = sponsor.BrandName || '';
    document.getElementById('sponsor-tier').value = sponsor.Tier || '';
    document.getElementById('sponsor-link').value = sponsor.Link || '';

    if (sponsor.LogoUrl) {
        const preview = document.getElementById('sponsor-logo-preview');
        preview.src = sponsor.LogoUrl;
        document.getElementById('sponsor-logo-name').textContent = 'current-logo';
        document.getElementById('sponsor-logo-size').textContent = 'Hosted URL';
        document.getElementById('sponsor-logo-preview-container').classList.remove('hidden');
    } else {
        clearSponsorLogo();
    }
...
async function deleteSponsor(id) {
    if (!confirm('Permanently delete this sponsor record?')) return;
    try {
        const response = await fetch(SCRIPT_URL, {
            method: 'POST',
            headers: { 'Content-Type': 'text/plain' },
            body: JSON.stringify({ action: 'deleteSponsor', data: { id: id } })
        });
        const result = await response.json();
        if (result.status !== 'success') throw new Error(result.message);
...
function resetSponsorForm() {
    ...
    document.getElementById('sponsor-cancel-btn').classList.add('hidden');
    clearSponsorLogo();
}
